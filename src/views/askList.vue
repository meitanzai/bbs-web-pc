...
<template>
    ...
    <!-- 标签 -->
    <div class="main-container wrap backgroundModule questionTabModule">
        <div @mouseleave="moreShrink()" @mouseenter="moreMouseIn">
            <div class="questionTab-box" >
                <div ref="headerNavigation" >
                
                    <router-link tag="a" to="/askList" :class="router.currentRoute.value.query.questionTagId == undefined || router.currentRoute.value.query.questionTagId == '' ? 'tab_current' :'tab'">全部</router-link>
                    
                    <template v-for="tag in state.tagList">
                        <router-link tag="a"  :to="{path: '/askList', query:{ questionTagId : tag.id}}" :class="router.currentRoute.value.query.questionTagId == tag.id ? 'tab_current' :'tab'">
                            {{tag.name}}
                        </router-link>

                        <!-- 二级标签 -->
                        <template v-for="childQuestionTag in tag.childTag">
				            <router-link :to="{path: '/askList', query: {questionTagId: childQuestionTag.id}}" :class="router.currentRoute.value.query.questionTagId == childQuestionTag.id ? 'tab_current' :'tab'" >{{childQuestionTag.name}}</router-link>            
                        </template>
...
<script lang="ts" setup>
    import { onMounted, ref ,nextTick, getCurrentInstance, ComponentInternalInstance, reactive, computed, } from 'vue'
    ...
    const { meta } = useMeta(
        computed(() => ({ 
            title:  '问答 - ' + store.state.title,
            meta: [
                {
                    name: 'keywords',
                    content: '问答 - ' + store.state.keywords
                },{
                    name: "description",
                    content: '问答 - ' + store.state.description
                }
            ]
        }))
    )
