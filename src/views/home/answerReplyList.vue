                ...
                <el-table :data="state.pageView.records" stripe class="dataTable">
                    <el-table-column align="center">
                        ...
                        <template #default="scope">
                            ...
                            <router-link tag="div" class="link" v-if="scope.row.status == 20" :to="{path: '/question', query:{ questionId : scope.row.questionId,answerId:scope.row.answerId,replyId:scope.row.id}}">
                                {{scope.row.content}}
                            </router-link>
                        </template>
                    </el-table-column>
...
<script lang="ts" setup>
    ...
    const { meta } = useMeta(
        computed(() => ({ 
            title:  '我的答案回复 - ' + store.state.title,
            meta: [
                {
                    name: 'keywords',
                    content: '我的答案回复 - ' + store.state.title
                },{
                    name: "description",
                    content: '我的答案回复 - ' + store.state.title
                }
