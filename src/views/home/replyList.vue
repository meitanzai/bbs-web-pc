                ...
                <el-table :data="state.pageView.records" stripe class="dataTable">
                    <el-table-column align="center">
                        ...
                        <template #default="scope">
                            ...
                            <router-link tag="div" class="link" v-if="scope.row.status == 20" :to="{path: '/thread', query:{ topicId : scope.row.topicId,commentId:scope.row.commentId,replyId:scope.row.id}}">
                                {{scope.row.content}}
                            </router-link>
                        </template>
                    </el-table-column>
...
<script lang="ts" setup>
    ...
    const { meta } = useMeta(
        computed(() => ({ 
            title:  '我的回复 - ' + store.state.title,
            meta: [
                {
                    name: 'keywords',
                    content: '我的回复 - ' + store.state.title
                },{
                    name: "description",
                    content: '我的回复 - ' + store.state.title
                }
